#!/usr/bin/env python3
"""Generate Task1 labels and a submission-ready JSON for Codabench."""
...
# ===== Config (edit here) =====
CKPT_PATH = THIS_DIR / "runs" / "semi_supervised" / "checkpoints" / "best_model.pt"
DATA_DIR = REPO_ROOT / "data" / "reference_data" / "t1_ct" / "val" / "images"
SUBMISSION_TASK_DIR = THIS_DIR.parent / "submission" / "t1_ct"
...
def discover_images(folder: str | Path) -> List[Dict[str, str]]:
    ...
    image_files = sorted(root.glob("*.nii.gz"))
    if not image_files:
...
def _load_state_dict(model: torch.nn.Module, ckpt_obj: Dict) -> None:
    if "student_state_dict" in ckpt_obj:
        state = ckpt_obj["student_state_dict"]
...
def main() -> int:
    ...
    model_size = str(train_args.get("model_size", "base"))
