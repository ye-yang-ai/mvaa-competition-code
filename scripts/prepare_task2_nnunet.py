#!/usr/bin/env python3
"""Prepare MVAA Task2 data as an nnU-Net v2 dataset."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-root", type=Path, default=Path("data/reference_data/t2_tee"))
    parser.add_argument("--nnunet-root", type=Path, default=Path("outputs/nnunet_task2_fold0"))
    parser.add_argument("--dataset-id", type=int, default=102)
    parser.add_argument("--dataset-name", type=str, default="MVAA_Task2")
...
def validate_label(path: Path) -> None:
    ...
    bad = [int(v) for v in values.tolist() if int(v) not in (0, 1, 2)]
    if bad:
        raise ValueError(f"Label {path} has unexpected values: {bad[:10]}")
...
def main() -> int:
    ...
    train_images = sorted((data_root / "train").glob("*-US.nii.gz"))
    val_images = sorted((data_root / "val" / "images").glob("*-US.nii.gz"))
    if not train_images:
        raise FileNotFoundError(f"No training images found under: {data_root / 'train'}")
    if not val_images:
        raise FileNotFoundError(f"No validation/test images found under: {data_root / 'val' / 'images'}")

    train_mapping = []
    for image_path in train_images:
        case_id = image_path.name.removesuffix("-US.nii.gz")
        label_path = data_root / "train" / f"{case_id}-label.nii.gz"
    ...
    (args.nnunet_root / "task2_case_mapping.json").write_text(json.dumps(mapping, indent=2), encoding="utf-8")

    print(f"Prepared {len(train_mapping)} training cases and {len(test_mapping)} test cases.")
    print(f"Dataset: {dataset_dir}")
    print(f"Mapping: {args.nnunet_root / 'task2_case_mapping.json'}")
    return 0
